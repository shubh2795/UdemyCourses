...
public class CovidDataService {
    ...
    private final String covidCasesDataURL = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv";
    ...
    public String getCovidCasesDataURL() {
        return covidCasesDataURL;
    }
    ...
    public void fetchCovidData() throws IOException, InterruptedException {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create(getCovidCasesDataURL())).build();
    ...
    @PostConstruct
    @Scheduled(cron = "* * 2 * * *")
    public void fetchCovidRecoveries() throws IOException, InterruptedException {
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create(getCovidCasesDataURL())).build();
        HttpResponse response = client.send(request, HttpResponse.BodyHandlers.ofString());
        StringReader csvReader = new StringReader((String) response.body());
